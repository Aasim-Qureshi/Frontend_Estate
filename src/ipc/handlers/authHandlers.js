...
async function confirmSingleReport(win, reportId) {
  ...
  try {
    ...
    return {
      status: "FAILED",
      error: `Failed to load report ${reportId}: ${err?.message || err}`,
    };
  }
...
async function confirmReportsBatch(win, reportIds = []) {
  if (!win || win.isDestroyed()) {
    return {
      ...
      results: reportIds.map((id) => ({
        reportId: id,
        status: "FAILED",
        error: "Secondary window not available",
      })),
    };
  }
...
async function waitForSecondaryLogin(
  win,
  timeoutMs = 180000,
  intervalMs = 1500,
) {
...
const authHandlers = {
  ...
  async handleGetCompaniesRealEstate(event) {
    try {
      const result = await pythonAPI.auth.getCompaniesRealEstate();
      if (!result)
        return {
          status: "ERROR",
          error: "Failed to get real estate companies",
        };
      return { status: result.status, data: result.data };
    } catch (error) {
      console.error("[MAIN] Get real estate companies error:", error);
      return { status: "ERROR", error: error.message };
    }
  },
  ...
  async handleNavigateToCompany(event, company) {
    try {
      ...
      if (!result)
        return { status: "ERROR", error: "Failed to navigate to company" };
  ...
  async handleSetRefreshToken(event, opts = {}) {
    ...
    try {
      ...
      const shouldPersist =
        sessionOnly !== true &&
        Number.isFinite(persistForDays) &&
        persistForDays > 0;
      if (shouldPersist) {
  ...
  async handleOpenTaqeemLogin(event, opts = {}) {
    ...
    if (batchId) {
      try {
        ...
        if (
          ...
          Array.isArray(batchResult.reports)
        ) {
          ...
          return {
            status: "ERROR",
            error:
              batchResult?.error || `No reports found for batch ${batchId}`,
          };
        }
      } catch (err) {
    ...
    try {
      if (automationOnly) {
        ...
        if (automationResult?.status === "SUCCESS") {
          return {
            ...
            message:
              ...
              "Opened Taqeem login in automation browser",
        ...
        return {
          status: "ERROR",
          error: automationResult?.error || "Failed to open Taqeem login",
        };
      }
