...
const TaqeemProgressBar = ({ submitted, sent, approved }) => {
  const steps = [
    { id: "new", label: "New", color: "#94a3b8" },
    { id: "submitted", label: "Submitted", color: "#6366f1", done: submitted },
    { id: "sent", label: "Sent", color: "#3b82f6", done: sent },
    { id: "approved", label: "Approved", color: "#10b981", done: approved },
  ];
      ...
      {steps.map((step, i) => {
        ...
        return (
          <React.Fragment key={step.id}>
            ...
            <div
              className="flex items-center gap-1 rounded-full px-2 py-0.5 shrink-0 transition-all duration-300"
              style={{
                background: current
                  ? step.color + "18"
                  : done
                    ? step.color + "12"
                    : "transparent",
                border: `1px solid ${done ? step.color + (current ? "99" : "44") : "#e2e8f0"}`,
              }}
            >
              {done && !current && (
                <CheckCircle2
    ...
    if (queued.includes("submit")) {
      ...
      try {
        ...
        if (window?.electronAPI?.submitRealEstateReport) {
          await window.electronAPI.submitRealEstateReport(reportId, pdfPath);
        } else {
          console.log(
