...
const Tickets = ({ onViewChange }) => {
  ...
  const isAssignedToMe =
    selectedTicket &&
    resolveId(selectedTicket.assignedTo) === resolveId(user?._id);
  const canTakeTicket =
    isSupport &&
    selectedTicket &&
    (!selectedTicket.assignedTo || assignedPhone === "000");
  ...
  const upsertTicket = useCallback((updated) => {
    ...
    setTickets((prev) => {
      ...
      const next = exists
        ? prev.map((ticket) =>
            ticket._id === updated._id ? { ...ticket, ...updated } : ticket,
          )
        : [updated, ...prev];
  ...
  const loadTickets = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(response?.tickets)
        ? response.tickets
  ...
  useEffect(() => {
          ...
          prev.map((ticket) =>
            ticket._id === payload.ticketId
              ? {
                  ...
                  lastMessagePreview:
                    payload.lastMessagePreview || ticket.lastMessagePreview,
                  assignedTo:
    ...
    socket.on("ticket:message", (payload) => {
      ...
      if (payload.ticketId === selectedTicketIdRef.current) {
        setMessages((prev) =>
          prev.some((msg) => msg._id === payload._id)
            ? prev
            : [...prev, payload],
        );
      }
  ...
  const handleSendMessage = async () => {
    ...
    if (chatAttachments.length > 0) {
      ...
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/api/tickets/${selectedTicketId}/messages`,
          {
            method: "POST",
            headers: { Authorization: `Bearer ${token}` },
            body: formData,
    ...
    socket.emit(
      "ticket:message",
      { ticketId: selectedTicketId, body: trimmed },
      (ack) => {
        setSending(false);
        if (!ack?.ok) {
  ...
  const handleTake = async () => {
    ...
    try {
      const response = await window.electronAPI.apiRequest(
        "POST",
        `/api/tickets/${selectedTicketId}/take`,
        {},
        headers,
      );
      const updated = response?.ticket;
      ...
      const msg =
        err?.response?.data?.message || err.message || "Failed to take ticket";
      alert(msg);
    }
  };
        ...
        <section className="rounded-2xl border border-[color:var(--ticket-border)] bg-[color:var(--ticket-surface)] px-4 py-3 shadow-[0_12px_24px_rgba(15,23,42,0.08)]">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h1 className="mt-1 font-display text-lg text-[color:var(--ticket-ink)]">
                Tickets
              </h1>
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
            <div className="rounded-xl border border-slate-200 bg-white px-2.5 py-2">
              <div className="text-[9px] text-[color:var(--ticket-muted)]">
                الكل
              </div>
              <div className="mt-1 font-display text-[13px] text-[color:var(--ticket-ink)]">
                {tickets.length}
              </div>
            </div>
            <div className="rounded-xl border border-amber-200/70 bg-amber-50/60 px-2.5 py-2">
              ...
              <div className="mt-1 font-display text-[13px] text-amber-900">
                {ticketStats.waiting}
              </div>
            </div>
            <div className="rounded-xl border border-sky-200/70 bg-sky-50/60 px-2.5 py-2">
              <div className="text-[9px] text-sky-700">
                {STATUS_LABELS.in_support}
              </div>
              <div className="mt-1 font-display text-[13px] text-sky-900">
                {ticketStats.in_support}
              </div>
            </div>
            <div className="rounded-xl border border-emerald-200/70 bg-emerald-50/60 px-2.5 py-2">
              ...
              <div className="mt-1 font-display text-[13px] text-emerald-900">
                {ticketStats.open}
              </div>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-100/70 px-2.5 py-2">
              ...
              <div className="mt-1 font-display text-[13px] text-slate-800">
                {ticketStats.closed}
              </div>
            </div>
          </div>
          ...
          <aside className="space-y-4">
                ...
                <form onSubmit={handleCreateTicket} className="mt-3 space-y-2">
                  ...
                  <textarea
                    ...
                    placeholder={t("tickets.create.descriptionPlaceholder", {
                      defaultValue: "اكتب تفاصيل المشكلة...",
                    })}
                  />
                  <label className="block text-[9px] font-semibold text-[color:var(--ticket-muted)]">
                  ...
                  <input
                    ...
                    placeholder={t("tickets.create.subjectPlaceholder", {
                      defaultValue: "عنوان مختصر للتذكرة",
                    })}
                  />
            ...
            <div className="rounded-2xl border border-[color:var(--ticket-border)] bg-white/95 p-3 shadow-[0_12px_24px_rgba(15,23,42,0.08)]">
              <div className="flex items-center justify-between">
                <div className="text-[10px] font-semibold text-[color:var(--ticket-ink)]">
                  {isAdmin || isSupport
                    ? t("tickets.list.adminTitle", {
                        defaultValue: "All tickets",
                      })
                    : t("tickets.list.userTitle", {
              ...
              <div className="mt-2 max-h-[460px] space-y-2 overflow-y-auto pr-1">
                {loadingTickets && (
                  <div className="text-center text-[10px] text-[color:var(--ticket-muted)]">
                    Loading...
                  </div>
                )}
                ...
                {filteredTickets.map((ticket) => {
                  ...
                  const statusClass =
                    STATUS_STYLES[ticket.status] || STATUS_STYLES.waiting;
                  const ownerLabel =
                    ticket.createdBy?.phone || ticket.createdBy?.displayName;
                  return (
                    <button
                      key={ticket._id}
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span
                          className={`rounded-full border px-2 py-0.5 text-[8px] font-semibold ${statusClass}`}
                        >
              ...
              <div className="flex flex-1 flex-col items-center justify-center px-4 py-6 text-center">
                ...
                <p className="text-[10px] text-[color:var(--ticket-muted)]">
                  {t("tickets.empty.subtitle", {
                    defaultValue: "Pick a ticket to view the conversation.",
                  })}
                </p>
              </div>
            )}
            ...
            {selectedTicket && (
              <>
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="max-w-xl">
                      ...
                      <div className="mt-1.5 font-display text-[15px] text-[color:var(--ticket-ink)]">
                        {descriptionText ||
                          t("tickets.chat.descriptionPlaceholder", {
                            defaultValue: "بدون وصف حتى الآن",
                          })}
                      </div>
                      ...
                      {isAdmin && (
                        <div className="flex items-center gap-2">
                          <select
                            value={assignPhone}
                            onChange={(event) =>
                              setAssignPhone(event.target.value)
                            }
                ...
                <div className="flex-1 space-y-3 overflow-y-auto bg-slate-50/70 px-4 py-4">
                  ...
                  {messages.map((msg) => {
                    const isMine =
                      resolveId(msg.senderId) === resolveId(user?._id);
                    ...
                    return (
                      <div
                        key={msg._id}
                        className={`max-w-[72%] ${isMine ? "ml-auto" : "mr-auto"}`}
                      >
                        <div
                        ...
                        >
                          <div
                            className={`mb-1 text-[8px] ${isMine ? "text-white/70" : "text-slate-400"}`}
                          >
                        ...
                        <div
                          className={`mt-1 text-[8px] text-slate-400 ${isMine ? "text-right" : "text-left"}`}
                        >
