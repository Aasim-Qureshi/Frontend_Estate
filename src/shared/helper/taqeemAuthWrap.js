...
async function runPublicLogin(isAuth) {
  try {
    ...
    if (!loginFlow || loginFlow.status === "FAILED") {
      return {
        status: "FAILED",
        error: loginFlow?.error || "Unknown login failure",
      };
    }
...
function readUserFromStorage() {
  ...
  const storageCandidates = [window.sessionStorage, window.localStorage].filter(
    Boolean,
  );
...
function getCachedSnapshot(cachedUser = null) {
  ...
  const profile = userData?.taqeem?.profile || userData?.profile || null;
  const companies = Array.isArray(userData?.taqeem?.companies)
    ...
    : Array.isArray(userData?.companies)
      ...
      : [];
  const taqeemUser =
    extractTaqeemUsernameFromUser(userData) || extractTaqeemUsername(profile);
  return {
    user: userData,
    profile,
    companies,
...
async function maybeSyncCachedSnapshot({
  ...
  // IMPORTANT: do NOT send `companies` here. This cached snapshot can be stale
  // (it isn't refreshed after ValueNavContext.syncCompanies() writes fresh data
  // to the DB), and posting it back would silently overwrite newer server data.
  // This call exists only to keep the taqeemUser/profile link alive.
  ...
  try {
    ...
    if (bootstrapResponse?.status === "TAQEEM_ALREADY_USED") {
      ...
      if (conflictReason === "SYSTEM_LOGIN_REQUIRED") {
        ...
        setTaqeemStatus?.(
          "error",
          bootstrapResponse?.message || "Taqeem account is already linked.",
        );
      }
    ...
    if (payload?.status === "TAQEEM_ALREADY_USED") {
      ...
      if (conflictReason === "SYSTEM_LOGIN_REQUIRED") {
        ...
        setTaqeemStatus?.(
          "error",
          payload?.message || "Taqeem account is already linked.",
        );
      }
      return payload;
    }
...
async function bootstrapAndSync({
  ...
  if (loginFlow?.status !== "CHECK") {
    console.info(
      "[taqeemAuth] bootstrapAndSync: publicLogin did not return CHECK",
      loginFlow,
    );
    return loginFlow;
  }
  ...
  console.info(
    "[taqeemAuth] bootstrapAndSync: manual Taqeem login CHECK received from worker",
    {
  ...
  const handleConflict = (payload = {}) => {
    ...
    emitTaqeemConflict({
      ...payload,
      taqeemUser:
        payload?.taqeemUser || payload?.username || resolvedTaqeemUser || null,
    });
    if (conflictReason === "SYSTEM_LOGIN_REQUIRED") {
      setTaqeemStatus?.("success", "Taqeem login: On");
    } else {
  ...
  const syncResult = await syncTaqeemSnapshot({
    ...
    skipCompaniesFetch:
      Array.isArray(cachedSnapshot.companies) &&
  ...
  if (
    ...
    const syncUser =
      syncResult?.user || buildLoginPayloadFromResponse(bootstrapResponse);
...
async function ensureTaqeemAuthorized(
  ...
  try {
    ...
    const needsFreshTaqeemLogin =
      ...
      (browserExplicitlyNotLoggedIn ||
        (!browserSessionLikelyAlive && !isTaqeemLoggedIn));
    ...
    if (authStatus?.status === "AUTHORIZED") {
      ...
      void maybeSyncCachedSnapshot({
        ...
        console.warn(
          "[taqeemAuth] maybeSyncCachedSnapshot (background) failed:",
          err?.message || err,
        ),
      );
      setTaqeemStatus?.("success", "Taqeem login: On");
    ...
    if (conflictData?.status === "TAQEEM_ALREADY_USED") {
      ...
      if (conflictReason === "SYSTEM_LOGIN_REQUIRED") {
        ...
        setTaqeemStatus?.(
          "error",
          conflictData?.message || "Taqeem account is already linked.",
        );
      }
      return conflictData;
    }
